...
source("D:/Downloads/Dengue districtly/Github code/01_load_packages_data.R")
...
models <- lapply(1:length(formulas), 
                 function(i) {
                   model <- mymodel(formulas[[i]], data = df2, family = "binomial", Ntrials = df$Total.traps)
                   save(model, file = paste0("model/aoi inla logit/", lab[i],".RData"))})
...
for(i in 1:length(formulas)){
  load(paste0("model/aoi inla logit/",lab[i],".RData"))
...
fwrite(table_aoi, file = "model/aoi inla logit/aoi model selection.csv", quote = FALSE, 
       row.names = FALSE)
...
formula0.3 <- Y2 ~ 1 + basis_mt
formula0.4 <- Y2 ~ 1 + basis_tr

formula1.1 <- Y2 ~ 1 + f(T2, model = "iid") + basis_tr
formula1.2 <- Y2 ~ 1 + f(T2, model = "iid") + basis_mt
formula1.3 <- Y2 ~ 1 + basis_mt + basis_tr
formula1.4 <- Y2 ~ 1 + f(T1, model = "iid") + f(T2, model = "iid")
formula1.5 <- Y2 ~ 1 + f(T1, model = "iid") + basis_tr
formula1.6 <- Y2 ~ 1 + f(T1, model = "iid") + basis_mt
formula1.7 <- Y2 ~ 1 + f(T1, model = "iid") + f(S, model = "iid")
formula1.8 <- Y2 ~ 1 + f(T2, model = "iid") + f(S, model = "iid")

formula2.1 <- Y2 ~ 1 + f(T1, model = "iid") + f(T2, model = "iid") + basis_tr
formula2.2 <- Y2 ~ 1 + f(T1, model = "iid") + f(T2, model = "iid") + basis_mt
formula2.3 <- Y2 ~ 1 + f(T1, model = "iid") + f(T2, model = "iid") + f(S, model = "iid")
formula2.4 <- Y2 ~ 1 + f(T1, model = "iid") + f(T2, model = "iid") + basis_tr + basis_mt
formula3 <- Y2 ~ 1 + f(T1, model = "iid") + f(T2, model = "iid") + basis_tr + basis_mt + f(S, model = "iid")
...
models <- lapply(1:length(formulas), 
                 function(i) {
                   model <- mymodel(formulas[[i]], data = df2, family = "nbinomial")
                   save(model, file = paste0("model/Number of mosquitoes/nb/", lab[i],".RData"))})
...
for(i in 1:length(formulas)){
  load(paste0("model/Number of mosquitoes/nb/",lab[i],".RData"))
...
models <- lapply(1:length(formulas),
                 function(i) {
                   model <- mymodel(formulas[[i]], data = df1, family = "Poisson")
                   save(model, file = paste0("model/Number of mosquitoes/po/", lab[i],".RData"))})

for(i in 1:length(formulas)){
  load(paste0("model/Number of mosquitoes/po/",lab[i],".RData"))
...
best.fit <- which.min(table0$logscore)
# Write results of model selection
fwrite(table_po, file = "model/Number of mosquitoes/nummosquitoes_model_selection(poisson).csv", quote = FALSE, 
       row.names = FALSE)
fwrite(table_nb, file = "model/Number of mosquitoes/nummosquitoes_model_selection(nbinomial).csv", quote = FALSE, 
       row.names = FALSE)
...
models <- lapply(1:length(formulas), 
                 function(i) {
                   model <- mymodel(formulas[[i]], data = df3, family = "binomial", Ntrials = df3$Total.traps)
                   save(model, file = paste0("model/aoi inla logit/full model/", lab[i],".RData"))})
...
# Write results of model selection
fwrite(table_aoi1, file = "model/aoi inla logit/aoi model selection/full model/aoi full model selection.csv", quote = FALSE, 
       row.names = FALSE)